...
class LocaEasyAcademySupabaseUser extends BaseAuthUser {
  LocaEasyAcademySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> locaEasyAcademySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LocaEasyAcademySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
